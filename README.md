...
9. **Event streaming integration**: Replace the direct HTTP ingestion endpoint with a Kafka consumer for `BookingCompleted` events. This would provide durable event delivery, replay capability, backpressure handling, consumer-group scaling, and better decoupling between the booking platform and settlement service. The current HTTP endpoint was chosen to keep the assignment lightweight and easy to run locally.
